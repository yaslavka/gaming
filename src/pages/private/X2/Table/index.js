...
export default function X2Table({ location: { state = {}, pathname } }) {
  ...
  const buyMatrix = () => {
    if (matrixInfo && matrixInfo.id) {
      ...
      api
        .buyMatrixUnos(Number(matrixInfo.id))
        .then((response) => {
          setBuyingStatus({
            type: 'success',
          ...
          api
            .getMatrixUnosStructureByType(matrixInfo.id)
            .then((response) => {
              if (response.items) {
  ...
  const closePartnerModal = () => {
    ...
    setTimeout(() => {
      ...
      if (pathname.startsWith('/personal-x2')) {
        api
          .getMatrixUnosStructureByType(matrixInfo.id)
          .then((response) => {
            if (response.items) {
    ...
    if (matrixInfo && matricesList) {
      if (direction === 'left') {
        if (matrixInfo.id !== 1) {
          ...
          history.push(`/personal-x2/${matrixInfo.id - 1}`)
        }
      } else {
        if (matrixInfo.id !== 8) {
  ...
  const backRouteElement = useMemo(() => {
    let returnRoute = (
      <Link to={routes.x2} className={styles.close}>
  ...
  const navRoute = useCallback(
    (route = '') => {
      ...
      if (matrixInfo && matrixInfo.isActive && !id) {
        newRoute = `/personal-x2/${matrixInfo.id}${route}`
      } else if (id) {
        newRoute = `/x-2/${id}${route}`
      }
      return newRoute
    },
    [matrixInfo, id],
      ...
      <div className="breadcrumbs flex items-center relative bg-no-repeat bg-scroll bg-center bg-cover lg:mb-32 md:mb-20 mb-15 h-365 md:h-460 lg:h-500">
        <div className="container">
          <div className="relative 2xl:pt-20 lg:pt-16 pt-12 md:mb-12 z-10 flex flex-col justify-center items-center text-center">
            {matrixInfo && <h1 className="text-white font-metal uppercase font-normal lg:text-5.5xl md:text-7xl text-5xl tracking-wide mb-6 text-shadow">personal x2 - {matrixInfo.name}</h1>}
            <ul className="flex">
              <li><a href="/"
              ...
              <li><span className="text-primary uppercase text-base sm:text-lg font-bold">personal-x2</span></li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="breadcru flex items-center relative bg-no-repeat bg-scroll bg-center lg:mb-32 md:mb-20 mb-15 h-620 md:h-650 lg:h-780">
        <Container style={{width: '90%'}}>
          <div className={styles.header}>
            {matrixInfo &&  <MyViewElement element={<h1 className={styles.title}>x2 - {matrixInfo.name}</h1>}/>}
            {backRouteElement}
          </div>

...
export default function PartnersClonesModal({ onClose }) {
  ...
  useEffect(() => {
    if (matrixInfo) {
      api
        .getMatrixUnosClonesCout(matrixInfo.id)
        .then((response) => {
          if (response.count > -1) {
          ...
          <div className={styles.box}>
            <div className={styles.header}>
              ...
              <h3 className={styles.title}>
                <span>{matrixClones || 0}</span>{t('Клонов')}
              </h3>
            </div>
            <button  disabled={setupClones}
                     onClick={installClones}
                     className="HashLink bg-no-repeat form-btn group primary-btn opacity-100 transition-all uppercase">
              Установить

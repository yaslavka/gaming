...
export default function GlieseTable({ location: { state = {}, pathname } }) {
  ...
  const backRouteElement = useMemo(() => {
    let returnRoute = (
      <Link to={routes.gliese} className={styles.close}>
  ...
  const navRoute = useCallback(
    (route = '') => {
      ...
      if (matrixInfo && matrixInfo.isActive && !id) {
        newRoute = `/personal-Gliese/${matrixInfo.id}${route}`
      } else if (id) {
        newRoute = `/Glieses/${id}${route}`
      }
      return newRoute
    },
    [matrixInfo, id],
      ...
      <div className="breadcrumbs flex items-center relative bg-no-repeat bg-scroll bg-center bg-cover lg:mb-32 md:mb-20 mb-15 h-365 md:h-460 lg:h-500">
        <div className="container">
          <div className="relative 2xl:pt-20 lg:pt-16 pt-12 md:mb-12 z-10 flex flex-col justify-center items-center text-center">
            {matrixInfo && <h1 className="text-white font-metal uppercase font-normal lg:text-5.5xl md:text-7xl text-5xl tracking-wide mb-6 text-shadow">personal Gliese - {matrixInfo.name}</h1>}
            <ul className="flex">
              <li><a href="/"
              ...
              <li><span className="text-primary uppercase text-base sm:text-lg font-bold">personal-Gliese</span></li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="breadcru flex items-center relative bg-no-repeat bg-scroll bg-center lg:mb-32 md:mb-20 mb-15 h-620 md:h-650 lg:h-780">
        <Container style={{width: '90%'}}>
          <div className={styles.header}>
            {matrixInfo &&  <MyViewElement element={<h1 className={styles.title}>Gliese - {matrixInfo.name}</h1>}/>}
            {backRouteElement}
          </div>

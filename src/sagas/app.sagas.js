...
export function* changeFinancePassword({ payload, callback }) {
  ...
  const toastMessage = `Финансовый пароль успешно ${hasFinPassword ? 'обновлен' : 'добавлен'}!`
...
export function* changeAutoRefillPlanets({ payload }) {
  try {
    ...
    if (response) {
      yield put(actions.changeAutoRefillPlanetsSuccess(payload.auto))

      toast.success(`Автоматическое обновление планет ${payload.auto ? 'включено' : 'выключено'}`)
    }
  } catch (error) {
...
export function* Promo({payload, callback}) {
  try {
    const response = yield call(api.Promo, payload);
    if (response) {
      yield put(actions.PromoSuccess(response));

      toast.success('Промо код успешно получен');
      callback && callback();
    }
  } catch (error) {
    yield put(actions.PromoError(error));

    toast.error(error.message);
  }
}
export function* PromoUser({payload, callback}) {
  try {
    const response = yield call(api.PromoSucces, payload);
    if (response) {
      yield put(actions.PromoUserSuccess(response));
      callback && callback();
    }
  } catch (error) {
    yield put(actions.PromoUserError(error));

    toast.error(error.message);
  }
}

export function* Stavki({payload, callback}) {
  try {
    const response = yield call(api.Stavki, payload);
    if (response) {
      yield put(actions.stavkiSuccess(response));

      toast.success('ставка успешно поставлена');
      callback && callback();
    }
  } catch (error) {
    yield put(actions.stavkiError(error));
...
export default function* mainSaga() {
  yield all([
    takeEvery(ActionTypes.CHANGE_PROMO_REQUES, PromoUser),
    takeEvery(ActionTypes.CHANGE_PROMO_REQUEST, Promo),
    takeEvery(ActionTypes.STAVKI_LIST_REQUEST, Stavki),
    ...
    takeEvery(ActionTypes.CHANGE_FINANCE_PASSWORD_REQUEST, changeFinancePassword),
    ...
    takeEvery(ActionTypes.PUBLISH_SUMMARY_TO_TELEGRAM_UPLOAD, publishSummaryToTelegram),
    takeEvery(ActionTypes.CHANGE_AUTO_REFILL_PLANET_REQUEST, changeAutoRefillPlanets),
  ])
}
